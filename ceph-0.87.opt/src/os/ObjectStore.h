  ...
  public:
    ...
    static void collect_contexts(
      ...
      for (list<Transaction *>::iterator i = t.begin();
	   i != t.end();
	   ++i) {
	if (out_on_applied) // + by tanghao
	  on_applied.splice(on_applied.end(), (*i)->on_applied);
	if (out_on_commit) // + by tanghao
	  on_commit.splice(on_commit.end(), (*i)->on_commit);
	if (out_on_applied_sync) // + by tanghao
	...
	*out_on_applied_sync = C_Contexts::list_to_context(on_applied_sync);
    ...
    // tolerate_collection_add_enoent(1) + ops(8) + pad_unused_bytes(8) + largest_data_len(4) +
    // largest_data_off(4) + largest_data_off_in_data(4) + __u32 + tbl + __u32 + data
	...
	largest_data_off = off;
        largest_data_off_in_data = data.length() ;//+ sizeof(__u32);
      }
      data.append(buf);
