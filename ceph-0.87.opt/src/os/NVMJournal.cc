...
void NVMJournal::dump_journal_stat()
{
    ...
    f->dump_int("total_hot_wrap", total_hot_wrap.read());
    f->dump_int("total_cold_wrap", total_cold_wrap.read());
...
void NVMJournal::put_object(ObjectRef obj, bool locked) 
{
    ...
    if (obj->alias.empty()) {
        ...
        if (0 == obj->put()) {
            ...
            if (enable_cache) {
		int index = reinterpret_cast<int64_t>(obj) % cache_slots;
...
void NVMJournal::adjust_heat_map()
{
	...
	jsize += (used - PAGE_SIZE*total_cached.read()) * 0.7;
        ...
        bufferlist& bl, uint64_t entry_pos, uint32_t boff, bool is_cold)
{
	...
	total_cached.sub(overlap/PAGE_SIZE);
	if (!is_cold)
	    total_cached.add(len/PAGE_SIZE);
        ...
        while (t != obj->data.end()) {
                    ...
                    && bh2->ext.end > bh->ext.start) 
            {
                ...
                if (bh2->bentry == bh->bentry) {
                    _flush_bh (obj, bh2);
		    uint32_t len = (bh2->ext.end - bh2->ext.start)/PAGE_SIZE;
		    total_evict.add(len);
		    total_flush.add(len);
		    total_cached.sub(len);
		    valid = true;
                }
            }// if 
            ++ t;
...
void NVMJournal::change_journal_mode()
{
    ...
    cached_data = PAGE_SIZE * cached_data / hot_journal_size;
    if (enable_hot_journal
	    && used > threshold
	    && cached_data > high)
    {
...
void NVMJournal::omap_cache_t::evict_omap(uint32_t pos)
{
    ...
    dout(5) << "pos = " << pos << ", count = " << count << dendl;
}
/*LRUCache*/
